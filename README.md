...
![workflow 실행해보기](./docs/05-enable-workflow.png)
...
- GitHub 는 저장소에 60일간 활동이 없으면 예약 워크플로를 자동으로 비활성화합니다. 이 저장소의 워크플로는 오래 변경이 없으면 스스로 keepalive 커밋을 남겨 활동을 유지하므로 신경 쓰지 않아도 됩니다. 단, fork 직후 Actions 를 한 번도 활성화하지 않으면 keepalive 도 돌 수 없으니 설정 3단계는 꼭 거쳐야 합니다.
